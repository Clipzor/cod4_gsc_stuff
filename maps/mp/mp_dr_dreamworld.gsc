...
musicmenu()
{
    ...
    self.hud_music[i] setText("DW|CLAN"); //title
...
{
    wait 10;
	iPrintLnBold("^2Welcome ^3To ^2mp_DreamWorld");
	
    while(1)
    {
		iPrintLn("^6Map Made By ^2Pro");
		iPrintLn("^3Pro ^2Discord : ^3Pro#1405");
        wait 10;
		iPrintLn("^1Report Any Bug !!!");
		wait 10;
        iPrintLn("^3Map ^2Made For ^3DW|Deathrun");
        wait 10;
        iPrintLn("^2Map Made By ^3Pro");
        wait 10;
        iPrintLn("^3Pro Discord : ^2Pro#1405");
		wait 10;
    }
}
...
{
givesnip = getEnt("sniper","targetname");
...
trap1()
{
	...
	trig setHintString("^2USE ^3T^0o ^3A^0ctivate^2!");
	trig waittill("trigger",player);
	trig setHintString("^1!^2A^3ctivate^2d^1!");

	for(;;)
	{
	trap4_1 rotatepitch (900,3,0.1);
	wait 3;
	trap4_2 rotatepitch (900,3,0.1);
	wait 3;
	trap4_3 rotatepitch (900,3,0.1);
	wait 3;
	trap4_4 rotatepitch (900,3,0.1);
	wait 3;
	trap4_5 rotatepitch (900,3,0.1);
	wait 4;
	}
}
...
tele2 = getEnt("origin_teleport1","targetname");
...
tele6 = getEnt("origin_teleport4","targetname");
...
sniproom()
{
	...
	while(1)
	{
        ...
        {
		    player room_setup(jump,"xm_mp","remington700_mp","yes",1);
            level.activ room_setup(acti,"remington700_mp","xm_mp","yes",1);
...
respectPluginCheck(player) { //support for _respect plugin
	if (level.finishPosition[level.playerEnterNum].guid != player.guid || level.inRoomPlugin) {
		...
		player setOrigin((-154, 10785, 76));
		player setPlayerAngles((87, 6, 0));
		return false;
	}
