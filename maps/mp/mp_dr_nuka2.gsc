...
wait 15;
lootje delete();
iprintlnbold("^8Start door opened. ^3:>");
wait 2;
}
}
...
secretopen()
{
	...
	iPrintLnBold("^2What now^3? ^2c:");
	trig2 show();
	trig2 waittill("trigger", player);
	iPrintLnBold("^2You can now enter^3. ^2:p");
	...
	wait 5;
	iPrintLnBold("^6She gave me idea about her name ontop ^3:>");
}
...
trigger = GetEnt( "end", "targetname" );
	...
	trigger waittill ("trigger", user);
		{
		iprintlnbold( "^7 " + user.name + " ^3 has finished first !");
		wait 0.1;
		}
	}
}
...
iPrintlnBold( " ^3" + player.name + " ^7has entered ^7the ^3game ^7selection !!!" );
...
iPrintlnBold("^3"+self.name+" ^7has ^1DIED"); 
iPrintlnBold("^3Selection Room ^7is ^7now ^1OPEN^7!!"); 
}
...
sniper()
{
	...
	for(;;)
	{
		...
		thread createhud(player.name + " ^3selected ^7snipah ^4war!");
		//noobsnip triggerOn();
		wait 5;
		player iprintlnbold("^3FIGHT ^7!!!");
		activator iprintlnbold("^3FIGHT ^7!!!");
...
knife() //Knife room lel
{
	...
	for(;;)
	{
		...
		thread createhud(player.name + " ^3Selected ^7Knife ^3room^7!!");
		wait 5;
