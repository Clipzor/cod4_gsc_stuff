...
for(;;)
{
	...
	if(level.bouncejumperhasniper == 0)
	{
		...
		level.jumperbouncehud setText("Jumper^2(" + player.name + ")^1 Fail: ^3" + level.bouncejumperfail);
	}
	...
	if(level.bounceactihasniper == 0)
	{
		...
		level.actibouncehud setText("Activator^2(" + player.name + ")^1 Fail: ^3" + level.bounceactifail);
...
for(;;)
{
	...
	if(!isdefined(idiot.fakesecret[i]))
	{
		...
		{
			idiot iprintlnbold("^2Secret Step^3:^1 " + idiot.secretnumber + "^3/^1" + k + " ^2Complet!");
		}
	}	
}
}
...
for(;;)
{
	...
	thread createhud(player.name + " ^3select sniper fight!");
	wait 5;
