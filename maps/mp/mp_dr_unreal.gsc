...
trap3()
{
    trap3_ent = getent("trap3", "targetname");
    trig3 = getent("trap3_trig", "targetname");
    trap3_hurt = getent("trap3_caulk", "targetname");
    
    trap3_hurt linkto(trap3_ent);

    trig3 waittill("trigger");
    trig3 delete();
    {
        trap3_ent movez(-344, 2);
        trap3_ent waittill("movedone");
        wait 13;
        trap3_ent movez(344, 2);
        trap3_ent waittill("movedone");
        wait 4.5;
...
secretteleback()
{
	...
	for(;;)
	{
		...
		iPrintln( " ^7" + player.name + " ^5h^7as ^5F^7inished ^5t^7he ^5S^7ecret ^5R^7oom^5!!" );
	}
}
