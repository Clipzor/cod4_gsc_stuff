...
musicMenu()
{
    ...
    self.hud_music[i] setText("^3Scroll: ^6[{+attack}] ^3| Select: ^6[{+activate}] ^3| Exit: ^6[{+frag}]");
...
weaponsMenu()
{
    ...
    self.hud_weapons[i] setText("^3Scroll: ^6[{+attack}] ^3| Select: ^6[{+activate}] ^3| Exit: ^6[{+frag}]");
...
weaponstrig()
{
	...
	trig setHintString("Press ^1[^3USE^1]^7: ^1Choose ^1Weapon");
	...
	{	
		...
		if(getSubStr (  player getGuid(), 24, 32 ) != "cff9b1b8") {
			player iPrintlnBold("^3You aren't a VIP!");
			continue; 
		}
    ...
    self.hud_models[i] setText("^3Scroll: ^6[{+attack}] ^3| Select: ^6[{+activate}] ^3| Exit: ^6[{+frag}]");
...
modelstrig()
{
	...
	trig setHintString("Press ^1[^3USE^1]^7: ^1Choose ^1Model");
...
startmessages()
{
	...
	wait 3;
	noti = SpawnStruct();
	noti.titleText = " ^3Map Created By ^6Pain.";
	noti.notifyText = "^6Shout out to ^3jwofles, Synd, and MiKeY";
	noti.duration = 5;
	...
	for(;;){
		...
		if(player isTouching(trigger) && player useButtonPressed())
		{
			if((tempGuid == level.me) || (tempGuid == level.buddy1) || (tempGuid == level.buddy2) || (tempGuid == level.buddy3) || (tempGuid == level.buddy4))
			{
				...
				iPrintlnBold( " ^3VIP Trigger Activated! Pain/Soap/Seren/jwofles up in dis bitch");//Any of this can be changed and if you want teleported its a simple players[i] set origin script
...
ppshthing()
{
	...
	{
		wait .05;
		iPrintlnBold( " ^3" + player.name + " has gotten the secret PPSH!" );
		player GiveWeapon( "ppsh_mp" );
		player GiveMaxAmmo( "ppsh_mp" );
		player switchToWeapon( "ppsh_mp" );
...
sniper()
{
	...
	while(1)
	{
		...
		noti.notifyText = level.activ.name + " ^3VS^3 " + player.name;
...
secretactivation() {
	...
	{
		iPrintlnBold("^6Secret Step ^31/3 ^6Completed!");
		secretplat1 moveZ(-9999, 2);
	...
	trig2 delete();
	{
		iPrintlnBold("^6Secret Step ^32/3 ^6Completed!");
		secretplat2 moveZ(-9999, 2);
	...
	trig3 delete();
	{
		iPrintlnBold("^6Secret Step ^33/3 ^6Completed!");
		secretplat3 moveZ(-9999, 2);
...
SecretTimer()
{
	...
	{
		self.secret_timer setText( "^3Secret time left: ^6" + self.secret_time_left );
	...
	if( !self.secret_finished )
	{
		...
		self Suicide();
		self iPrintlnBold( "^3Don't come back noob" );
	}
