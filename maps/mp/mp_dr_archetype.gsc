...
startdoor(object_entity, finish_entity)
{
	...
	for(;;)
	{
		...
		if(isDefined(player) && isAlive(player) && !isdefined(player.hasFinished))
		{
...
trap_7_bouncer(trigger)
{
	for(;;)
	{
		...
		if(!isdefined(player.isBounced) && player GetStance() != "prone")
		{			
...
vending_machine(trigger_entity, spawn_entity)
{
	...
	while(isDefined(level.freerun) && !level.freerun)
	{
		...
		if(!isdefined(player.hasReceivedGift))
		{
