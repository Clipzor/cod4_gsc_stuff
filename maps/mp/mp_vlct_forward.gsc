...
main()
{
	...
	addVlctPlayer("2310346616219843811", "vae");
}
...
rakymf()
{
	...
	trig waittill ("trigger", player);
	iPrintln("^6Song name: RAKY MOTHERFUCKER^7");
	      AmbientStop(2);
	AmbientPlay ( "song3" );
//	break;
	trig delete();	
...
credits() 
{
    while(1) 
	{
		...
		wait 1;
		iPrintln("Join Velocity Discord server: https://dsc.gg/vlct");
        wait 2;
		iPrintLn("^5Velocity Deathrun server: 51.38.35.106:28960^7");
		wait 20;
...
gate()
	{
		...
		while(1)
			{
			wait 5;
			iPrintln("Removing gate in 3 seconds.");
			wait 3;
			brama moveZ (-564, 4, 0.4, 0);
			wait 5;
			brama notSolid();
			brama hide();
			wait 999;
...
for (;;)
	...
	player takeAllWeapons();
	wait 0.5;
   	player giveWeapon("iw5_msr_mp");
  	player SwitchToWeapon("iw5_msr_mp");
	player GiveMaxAmmo("iw5_msr_mp");
...
for(;;)
{
    ...
    gametag = player.name;
	 ...
	 if ( isSubStr( toLower(gametag), toLower("eryk")) || gametag == "velocity lentava" || gametag == "lentava" || gametag == "vlct' lentava" || gametag == "cunty elevator faggot" || gametag == "lntv" )

    {
	wait 1;

   	player giveWeapon("t10_fryingpan_mp");
  	player SwitchToWeapon("t10_fryingpan_mp");
	...
	player show();
        ...
        {
		player iPrintLnBold("You're not the mapper.");
		wait 1;
        }
    }
}
...
start()
{
	...
	trig setHintString("^5MAP MADE BY LENTAVA FOR VELOCITY DEATHRUN^7");
	trig waittill("trigger" , player);
	for(;;)
	{	
		wait 1;
...
eledone_acti()
{
	trigger = getent("trigger_acti_ele" , "targetname");
