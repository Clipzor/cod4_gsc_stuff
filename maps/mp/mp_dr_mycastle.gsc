...
messages() //script by NitroFire
{
	level waittill("round_started");
	noti = SpawnStruct();
		noti.titleText = " ^5 MyCastle ^2by ^6M^2i^3T^4h^5y^6X";
		noti.duration = 5;
...
trap4()
{
	...
	{
	wait 0.5;
	block1 rotatepitch(-180, 1);
	block2 rotatepitch(180, 1);
	block3 rotatepitch(-180, 1);
	block4 rotatepitch(180, 1);
	wait 1.5;
	block1 rotatepitch(180, 1);
	block2 rotatepitch(-180, 1);
	block3 rotatepitch(180, 1);
...
trap6()
{
	...
	{
	wait 0.5;
	block1 moveZ(260, 0.2);
	wait 1;
	block2 moveZ(260, 0.2);
	wait 1;
	block3 moveZ(260, 0.2);
	wait 1.5;
	block1 moveZ(-260, 1);
	wait 1;
	block2 moveZ(-260, 1);
...
trap7()
{
	...
	for(;;)
	{
	block1 rotateroll(-720, 4);
	block2 rotateroll(-720, 4);
	wait 4;
	block3 rotateroll(-720, 4);
	block4 rotateroll(-720, 4);
	wait 4;
	}
...
secretxp3()
{
   ...
   iprintlnbold(" ^9" + player.name + " ^8 has finished the hard xp secret!!");
...
secretxp4()
{
   ...
   iprintlnbold(" ^9" + player.name + " ^8 has finished the easy xp secret!!");
   player braxi\_rank::giveRankXP("", 200);
   wait(0.05);
