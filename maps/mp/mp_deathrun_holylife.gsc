...
start_elevator()
{
	...
	wait 8;//we wait 8 seconds before we move the platform up
	thread MoveSpawnsX();
...
trap3()
{
	...
	trig delete();
		for(;;)
		{
		brush rotateyaw(360, 3.5); //3 is the time it takes to do 1 rotation (so it's like the speed), change if you want
		wait 1;
		}
}
...
secret_respawn()
{
	...
	while(1)
	{
		...
		{
			player iPrintLnBold(" ^5You ^7Failed! ^3Respawned"); //You can use a msg but not needed :D
...
secret_respawn2()
{
	...
	while(1)
	{
		...
		{
			iPrintLnBold(player.name+ " ^5Has ^7Passed ^4HolyLife ^8Secret!^4!^5! " ); //You can use a msg but not needed :D
			player setOrigin( target.origin );
			player setPlayerAngles( target.angles );
		}
	}
...
/*dohud()
{
        ...
        color = randomInt(7);
             {
                case 0:
                        level.xxcolor = (0.5, 0.0, 0.8);
                        break;
                case 1:
                        level.xxcolor = (0.0, 0.8, 0.0);
                        break;
                case 2:
                        level.xxcolor = (0.9, 1.0, 0.0);
                        break;
                case 3:
                        level.xxcolor = (0.0, 0.0, 1.0);
                        break; 
                case 4:
                        level.xxcolor = (0.3, 0.0, 0.3);
                        break;
                case 5:
                        level.xxcolor = (0.5, 0.0, 0.2);
                        break;
                case 6:
                        level.xxcolor = (0.0, 1.0, 1.0);
                        break;
        }
 
        if(isDefined(level.xx))
                level.xx destroy();
 
        level.xx = newHudElem();       
        level.xx.x = 0;
        level.xx.y = 150;
        level.xx.horzAlign = "center"; 
        level.xx.vertAlign = "middle";
        level.xx.alignX = "center";
        level.xx.alignY = "middle";
        level.xx.sort = 102;   
        level.xx.font = "objective";
        level.xx.foreground = 1;       
        level.xx.archived = true;      
        level.xx.alpha = 1;    
        level.xx.fontScale = 2.5;
        level.xx.hidewheninmenu = false;       
        level.xx.color = (level.xxcolor);      
        level.xx.glowColor = (0.3, 0.6, 0.3);
        level.xx.glowAlpha = 1;
}
...
night()
{
   x = randomInt(6);
	  if(x == 0)
	 {
	 SetExpFog(200, 300, 0,0,0,0); 
	 level.xx setText("Night Mode");
	 }
