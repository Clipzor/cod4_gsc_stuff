/*
Map created by   FNRP-Muffinator      Coding: blu
   ...
   ¶¶________¶¶__¶¶_____¶¶__¶¶¶________¶¶
  ¶¶_______________¶¶__¶¶_____¶¶¶_______¶¶
...
{ 
	...
	rightdoor movey ( 340, 2 );
	leftdoor movey ( -340, 2 ); 
}
...
{ 
	...
	for(;;)
	{
		for (i = 0; i < twoblockkeys.size; i++) 
		{ 
			twoblockkeys[i] rotateRoll( 360, 1.5 );
			
		}	
			twoblockkeys[twoblockkeys.size -1] waittill ("rotatedone");
			wait 1;
	}
}
...
{ 
	...
	for (i = 0; i < longkeys.size; i++) 
	{ 
		longkeys[i] thread move (i);  
	}
...
trap4()
{
	...
	trig waittill ("trigger", player);
	trig delete();
	fly_trig = getEntArray ("fly_trig","targetname");
	speakerfly  delete();
	

		for (i = 0; i < fly_trig.size; i++)
		fly_trig[i] thread flyplayers();
}	
...
trap5()
{
	...
	for (;;) 
	{
		for (i = 0; i < yuhufly1.size; i++)
		{ 
			yuhufly1[i] rotateroll ( 360, 0.5);
		}
			yuhufly1[0] waittill ("rotatedone");
			wait 3;
		}
...
{ 
	...
	for(;;)
	{ 
		for (i = 0; i < flipit.size; i++)
		{
			flipit[i] rotatepitch ( 360, 1 );
			flipit[i] waittill ("rotatedone");
			wait 0.8;
		}
	}
}
...
trap7()
{
	...
	for(;;)
	{
		for (i = 0; i < rotatingkeys.size; i++)
		{ 
			rotatingkeys[i] rotatePitch ( 360,1 );
			rotatingkeys[i] waittill ("rotatedone");
			wait 2.5;
		}
	}
}
