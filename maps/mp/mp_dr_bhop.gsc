...
{ 
	...
	death settext("^7Map by ^5DC | ^0death");
...
/*teleport3()
{
   level.trig_rs = getent("trig_roomselection", "targetname");
   tele1 = getent ("origin_roomselection", "targetname");
   
   for(;;)
    {   
     level.trig_rs setHintString ("^0Press ^3[&&1]^0 to enter ^3Room Selection^0!");
     level.trig_rs waittill ("trigger", player);

     ///////RESPECT SCRIPT/////////////////////////
        
	    ///////RESPECT SCRIPT END/////////////////////////
		if( !isDefined( level.trig_rs ) )
		return;
...
if(level.firstenter == true)
{
	...
	acti iPrintLnBold("^3Jumper ^0is picking a ^3room ^0so don't ^3move^0!");
...
ss_check(player)
{
	...
	else //level.activ wins
	{
		...
		if (isDefined(level.activ))
		{
			...
			player iPrintLnBold("^1No level.activ ?!");
	}
}
...
{  
    while (1)
    {
	    ...
	    level.trigger_strafe setHintString ("^0Press ^3[&&1]^0 to enter ^3PureStrafe Room");
...
pureend()
{
    ...
    winner = getEnt("purewinner", "targetname");
    loser = getEnt("purelooser", "targetname");
    ...
    while (1)
    {
        ...
        if (player.pers["team"] == "allies") 
        {
            ...
            iPrintLn("^3Jumper: ^7" + level.race_jumper.name + " won the race!");
