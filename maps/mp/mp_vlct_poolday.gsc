...
trap2()
{
	...
	trap2_mortar_hurt thread maps\mp\_utility::triggerOff();
	trap2_trigger SetHintString("^5Press [USE] to activate trap!");
	trap2_trigger waittill("trigger", player);

 	trap2_trigger SetHintString("^1Mortar inbound!");
 	trap2_laptop SetModel("com_laptop_generic_open");

 	trap2_mortar_incoming PlaySound("artillery_incoming");
 	wait 1;
 	trap2_mortar_hurt thread maps\mp\_utility::triggerOn();
 	trap2_mortar_impact PlaySound("mortar_impact_water");
 	PlayFX(level._fx["Water"]["Trap2_Mortar_Water_Impact"], trap2_mortar_impact.origin);
 	PlayFX(level._fx["Wood"]["Trap2_Platform_Explode"], trap2_mortar_impact.origin);
 	trap2_mortar_hurt thread trap2_kill_players();
 	trap2_platform Hide();
 	trap2_platform NotSolid();
 	trap2_platform MoveTo(trap2_moveto_start.origin, 0.1);
 	wait 0.2;
 	trap2_mortar_hurt thread maps\mp\_utility::triggerOff();
 	trap2_trigger SetHintString("^1Trap activated!");
 	wait 2;
 	trap2_platform Show();
 	trap2_platform Solid();
 	trap2_platform MoveTo(trap2_mortar_impact.origin, 2);
 	wait 2;
 	thread trap2();
}
...
trap5()
{
	...
	trap5_trigger SetHintString("^5Press [USE] to activate trap!");
	trap5_trigger waittill("trigger", who);

	trap5_trigger SetHintString("^1Trap activated!");
 	trap5_laptop SetModel("com_laptop_generic_open");
...
trap6()
{
	...
	if(level.watermovedback == 0)
	{
		trap6_running_water MoveX(-700, 1);
		level.watermovedback = 1;
		wait 0.1;
	}

	trap6_water_bounce thread maps\mp\_utility::triggerOff();
	trap6_trigger SetHintString("^5Press [USE] to activate trap!");
	trap6_trigger waittill("trigger", player);

 	trap6_trigger SetHintString("^1Trap activated!");
 	trap6_laptop SetModel("com_laptop_generic_open");

 	trap6_water_bounce thread maps\mp\_utility::triggerOn();
 	trap6_water_bounce thread trap6_water_bounce();
 	trap6_running_water MoveX(700, 1);
 	trap6_water_sound PlaySound("trap6_watergush");
 	wait 11;
 	trap6_running_water MoveX(-700, 2);
 	trap6_water_bounce thread maps\mp\_utility::triggerOff();
 	wait 5;
 	thread trap6();
}
...
trap7()
{
	...
	wait 1;
	if(level.trap7triggerlinked == 0)
	{
		level.trap7_water_bounce EnableLinkTo();
		level.trap7_water_bounce LinkTo(level.trap7_target_ent);
		level.trap7triggerlinked = 1;
		wait 0.1;
	}
	level.trap7_water_bounce thread maps\mp\_utility::triggerOff();

	level.trap7_target_ent Hide();
 	level.trap7_target_ent NotSolid();

	wait 5;
	thread helicopter_spawn();
	wait 1;

	trap7_trigger SetHintString("^5Press [USE] to activate trap!");
	trap7_trigger waittill("trigger", player);

 	trap7_trigger SetHintString("^1Trap activated!");
 	trap7_laptop SetModel("com_laptop_generic_open");
