...
rooms_setup(room)
{
	...
	while(1)
	{
		...
		notif = SpawnStruct();
		notif.notifyText = "^3>> ^7"+player.name+" ^3Entered "+room.name+" room!";
		notif.duration = 4;
		...
		wait 1;
		player iPrintLnBold("^3GO!");
		level.activ iPrintLnBold("^3GO!");
...
secret_enter(secret)
{
	...
	for(;;)
	{
		...
		player iPrintLnBold("^3You entered the ^7"+secret.name+" ^3secret!");
	}
}
...
secret_finish(secret)
{
	...
	for(;;)
	{
		...
		player.secret_checkpoint = [];
		player.secret_checkpoint[secret.name] = undefined;
		iPrintLnBold(player.name+" ^3Finished the ^7"+secret.name+" ^7secret!");
	}
}
...
music_setup(music)
{
	...
	self.music_hud[i] setText(">^3>^7> ^7Select ^7a ^7Song ^3<^7<^3<");

	i++;
	self.music_hud[i] = braxi\_mod::addTextHud(self, 288, 360, 1, "center", "top", 1.4);
	self.music_hud[i].sort = 885;
	self.music_hud[i] setText("Press ^3[^7USE^3]^7: ^3Play ^7Song");
	...
	while(1)
	{
		...
		{
			iPrintLn("^3Now playing: ^7"+music[self.selection].name);
