...
waitTillTrigger( number )
{
        ...
        iPrintLn( "^5"+player.name+"^7 found the ^5"+real+"-Star^7 Dragonball" );
...
dragonballsfound()
{
        ...
        while(level.found<7)
        {
                ...
                {
                        trigger SetHintString( "Call the ^5Dragon Shenlong^7 and make your ^5Wish^7!" );
                        trigger waittill("trigger",player);
                        iPrintLn( "^5"+player.name+"^7 has called ^5Shenlong^7!" );
                        thread grantxp();
                        thread spawn_s();
                        ...
                        trigger SetHintString("Not all ^5Dragonballs^7 have been found yet!");
        }
}
...
hud()
{
        ...
        level.xxx.label = &"Collected Dragonballs:^5 &&1/7";
...
jumpfix()
{
	dbgt_jump = (6380,-5340,-1900);
	while(1)
	{
		players = getentarray("player", "classname");
		for(i=0;i<players.size;i++)
		{
			if( Distance( players[i].origin, dbgt_jump ) <= 120 && isAlive(players[i]))
				players[i] thread autobounce();
		}
		wait 2;
	}
}
...
enemey_low()
{
        ...
        if(isdefined(level.activ))
        {
                ...
                {
                        iprintlnbold("^5Buu's^7 Ghost has escaped!");
                        wait 5;
                ...
                {
                        iprintlnbold("^5Buu's^7 Ghost is banned by God.");
                        trigger delete();
                }
        }
        else
...
ki_attacks() // Rycoons Ruin2 Final Fight
{
        ...
        while(1)
        {
                ...
                if( self meleeButtonPressed() )
                {
                        ...
                        if( self.cur_spell > 2 )
                                self.cur_spell = 1;
                        self iprintln("Current KI-Attack: ^5"+yourPower(self.cur_spell));
                }
                if( delay > 0 )
                        delay --;
        ...
        if(isdefined(level.activ))
        {
            ...
            level.activ PlaySoundToPlayer( "tele", level.activ );
                player thread findburner();
                        player thread superpower();
                        iPrintLnBold(player.name+ " has entered the ^51End Fight");
...
countdown()
{
        ...
        while(1)
        {
                self iPrintLn("Press ^5Fire ^7to use your ^5KI-Attacks^7!");
                wait 5;
                self iPrintLn("Press ^5Knife ^7to change your ^5KI-Attacks^7!");
                wait 5;
                self iPrintLn("Press ^5Right Mouse ^7to transmit");
                wait 5;
        }
}
...
playersetup()
{
        ...
        self iPrintLnBold("You have become an ^3Super ^5Saiyan");
}
