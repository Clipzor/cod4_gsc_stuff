...
welcome()
{
	...
	wait 0.5;
	iPrintLnBold("mp_dr_skii By: ^3Lifezor");
}
...
creds()
{
	...
	for(;;)
	{
		...
		self.logoText setText("^3->^4Map Made By Lifezor^3<-");
		wait 5;
		self.logoText fadeOverTime(1);
		self.logoText.alpha = 0;
		wait 1;
		self.logoText fadeOverTime(1);
		self.logoText.alpha = 1;
		self.logoText setText("^3->^4Steam:/id/LfezR^3<-");
		wait 5;
		self.logoText fadeOverTime(1);
		self.logoText.alpha = 0;
		wait 1;
		...
		if(level.firstenter==false)
		{
		 ...
		 level.firstenter=true;
			...
			player FreezeControls(1);
					...
					noti = SpawnStruct();
					noti.titleText = (player.name + "^3Entered^3Sniper ^2Fight!");
		...
		if(level.firstenter==false)
		{
		 ...
		 level.firstenter=true;
			...
			player FreezeControls(1);
					...
					noti = SpawnStruct();
					noti.titleText = (player.name + "^3Entered ^3Knife ^2Fight.");
...
hardsecret()
{
	...
	while(1)
	{
		...
		iprintlnbold("^31 Of 4..");
		trig1 delete();
		trig2 waittill("trigger", player);
		iPrintLnBold("^32 Of 4..");
		trig2 delete();
		trig3 waittill("trigger",player);
		iPrintLnBold ("^33 Of 4..");
		trig3 delete();
		trig4 waittill("trigger", player);
		door notsolid();
		iPrintLnBold("^34 Of 4..");
		wait 0.5;
...
secretend()
{
	...
	while(1)
	{
		...
		
		{
			player setOrigin( target.origin );
			player setPlayerAngles( target.angles );
			//iPrintLnBold(player.name "Finished ^3Secret^7!");
