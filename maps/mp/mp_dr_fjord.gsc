...
vcltfunc() {
	...
	self setClientDvar("cg_thirdpersonangle" , "170");
	self setClientDvar("cg_thirdpersonrange" , "105");
	self setViewModel("vh_codo_cyberfemale");
	self giveWeapon("deserteagle_mp");
	self SwitchToWeapon("deserteagle_mp");
	self GiveMaxAmmo("deserteagle_mp");
	wait 1;
	self giveWeapon("h1_karambit_mp");
	self switchToWeapon("h1_karambit_mp");
	iPrintLnBold("Welcome " + self.name + " on server.");
	iPrintLN(self.name + " took his stuff.");
	self hide();
	wait 5;
	self iPrintLnBold("Karambit & Cyber Female made by SadSlothXL");
...
sctp() {
	...
	for(;;)
	{
		...
		player setOrigin(tele.origin);
		player setPlayerAngles(tele.angles);
		player thread sctp_thread();
	}
}

sctp_thread() {
	self freezeControls(1);
	self iPrintLnBold("Wait...");
	wait 12;
	self freezeControls(0);
	wait 3;
