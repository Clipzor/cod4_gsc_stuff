...
on_final_check()
{
	for(;;)
	{		
		if(level.jumpers == 1)							// THX Arfy <3
		{
			iPrintLnBold("^6" + level.jumper[0].name + " ^7is the ^6final ^7jumper!");
			level.jumper[0] braxi\_rank::giveRankXp("trap_activation");
			level.playerLastAlive = level.jumper[0];	// level.jumper[0] = last alive
		}
...
{	
	...
	while(!isDefined(level.activ))						// waits for activator to get spawned
	{
		if(level.freerun)								// so the door opens in freerun
			break;
	...
	for(i = 0; i < players.size; i++)
	{
		...
		players[i].hasAmmo = false;						// has got ammo
	}
	
	if(!level.freerun)
			thread on_final_check();
...
on_startdoor(object_entity)
{
	...
	thread on_startdoor_opened("start_door_icomar", "spawn_wall", "trigger_ueber_startdoor", "startdoor_origin_wall");
	wait 3;
	...
	iPrintLn("> Credits to ^6Blade ^7for the sick teleport FX");
	
	startdoor delete();
}

on_startdoor_opened(trigger_1_entity, wall_1_entity, trigger_2_entity, wall_2_entity)
{
	trigger = getEnt(trigger_1_entity, "targetname");
	trigger2 = getEnt(trigger_2_entity, "targetname");
	wall1 = getEnt(wall_1_entity, "targetname");
	wall2 = getEnt(wall_2_entity, "targetname");
