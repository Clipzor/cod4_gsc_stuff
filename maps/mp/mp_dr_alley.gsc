...
start_door()
{
	...
	if(i == 0)
	{
		ambientPlay("laszlo");
		wait 10;
		iprintln("^3Song: ^7Fall To Light - Laszlo");
		object movez(-120,2);
	}
	else if(i == 1)
	{
		ambientPlay("aero");
		wait 10;
		iprintln("^3Song: ^7Time Leap - Aero Chord");
		object movez(-120,2);
	}
}
map_mover()
...
trap_stairs()
{
	...
	trigger SetHintString("^3Activated");
	object delete();
}
trap_pusher()
{
...
rpg_room()
{
	...
	while(1)
	{
		...
		if(!isdefined(level.entertext)||!level.entertext)
		{
			...
			iprintln("^1Now playing: ^7Audioscribe^3- ^7Free fall");
		}
...
{
	wait 9;
	iPrintlnBold("^7Map by ^3TheLixfe");
	wait 2;
