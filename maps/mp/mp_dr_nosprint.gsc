...
secret(secret_org) {
	for(;;) {
		self waittill("trigger", player);
		player setorigin(secret_org.origin);
		player setplayerangles(secret_org.angles);
	}
}
...
main_fall(main_fall_org) {
	for(;;) {
        ...
        if(!isdefined(player.deathcount_nosprint)) {
            player.deathcount_nosprint = 2;
            player thread player_reset_deathcount_die();
        }
        
        if(player.deathcount_nosprint > 0) {
            player setvelocity((0,0,0));
            player setorigin(main_fall_org.origin);
            player setplayerangles(main_fall_org.angles);
            player iprintln("Tries Left Before Death: ^2" + player.deathcount_nosprint);
            player.deathcount_nosprint--;
        } else {
            player suicide();
            player iprintln("No More Tries Left, You Die");
            player.deathcount_nosprint = undefined;
        }
	}
}
