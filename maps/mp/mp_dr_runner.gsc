...
onPlayerConnect()
{
    for(;;)
    {
        level waittill( "connected", player );
                  player iPrintLnBold("^4Welcome runner ^3"+player.name);
    }
}
...
wait 10;
fence moveZ(-300, 3);
iprintlnBold("^4Starting ^3Fence ^6Lowered.");
wait 1;
}
...
for(;;)
{
	if(self istouching(b2))
	{
		...
		IPrintLnbold("^3" + self.name + "^4 Is a Super boi^6!");
...
xp()
{
	...
	player iprintlnBold ("^1You ^2Have ^3Found ^4The ^3Secret ^6XP^7!");
	trig delete();
}
...
old()
{
	...
	while (1)
	{
		level.trigger_old SetHintString ("^4Old ^3Room");
...
sniper()
{
	...
	while (1) 
	{
		level.trigger_sniper SetHintString ("^4Sniper ^3Room");
...
bounce()
{
	...
	while (1) 
	{
		level.trigger_bounce SetHintString ("^4Bounce ^3Room");
