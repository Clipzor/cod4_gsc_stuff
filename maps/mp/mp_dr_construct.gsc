...
main() {
    ...
    preCacheModel( "t6_zombie_body" );
    preCacheItem( "zomb_mp" );
...
secret_enter() {
    ...
    goto = getEnt( "secret_enter_org", "targetname" );

    while( 1 ) {
        trig waittill( "trigger", p );
        iPrintLnBold( p.name + " found bub the buildr" );
...
trap2() {
    ...
    randomPart = randomInt( 2 );
        switch( randomPart ) {
        case 0:
        for( i = 0; i < drop1.size; i++ ) {
            drop1 moveZ( -370, 4, 1, 1);
            wait .05;
        }
        break;
        case 1:
        for( i = 0; i < drop2.size; i++ ) {
            drop2 moveZ( -370, 4, 1, 1);
            wait .05;
        }
        break;
    }
...
trap3() {
    ...
    plat1 moveZ( -370, 4, 1, 1 );
    wait .5;
    plat2 moveZ( -370, 4, 1, 1 );
    wait .5;
    plat3 moveZ( -370, 4, 1, 1 );
    wait .5;
    plat4 moveZ( -370, 4, 1, 1 );
    wait 9;
    plat1 moveZ( 370, 4, 1, 1 );
    wait .5;
    plat2 moveZ( 370, 4, 1, 1 );
...
trap4() {
    ...
    log1 rotateRoll( -1080, 5, 1, 1 );
    log2 rotateRoll( 1080, 5, 1, 1);
    wait 5;
    log1 rotateRoll( 1080, 5, 1, 1 );
    log2 rotateRoll( -1080, 5, 1, 1);
}
...
trap8() {
    ...
    log1 moveZ( -140, 2, 1, 1 );
    wait .5;
    log2 moveZ( -140, 2, 1, 1 );
    wait .5;
    log3 moveZ( -140, 2, 1, 1 );
    wait .5;
    log4 moveZ( -140, 2, 1, 1 ); 
    wait 5;
    log1 moveZ( 140, 2, 1, 1 );
    wait .5;
    log2 moveZ( 140, 2, 1, 1 );
...
trap9() {
    ...
    onea rotateYaw( -720, 3, 1, 1 );
    oneb rotateYaw( -720, 3, 1, 1 );
    wait 1;
    twoa rotateYaw( 720, 3, 1, 1 );
    twob rotateYaw( 720, 3, 1, 1 );
    wait 1;
    threea rotateYaw( -720, 3, 1, 1 );
    threeb rotateYaw( -720, 3, 1, 1 );
    wait 5;
    onea rotateYaw( 720, 3, 1, 1 );
    oneb rotateYaw( 720, 3, 1, 1 );
    wait 1;
    ...
    while( 1 ) {
        ...
        level.zombie setHintString( "^8" + p.name + " ^7is in the zombie room with ^8" + level.activ.name + "^7!" );  // change to level.activ
            ...
            level.inKnife = true;
                ...
                p GiveWeapon( "zomb_mp" );
                p switchToWeapon( "zomb_mp" );
                p.maxhealth = 100; 
                ...
                if( isDefined( level.activ ) && isAlive( level.activ ) ) {
                    ...
                    level.activ GiveWeapon( "zomb_mp" );
                    level.activ switchToWeapon( "zomb_mp" ); 
...
bounce_grab() {
	...
	while( 1 ) {
		...
		player giveWeapon( "r700_noscope_mp" );
		player giveMaxAmmo( "r700_noscope_mp" );
		player switchToWeapon( "r700_noscope_mp" );
		player giveWeapon( "m40a3_noscope_mp" );
		player giveMaxAmmo( "m40a3_noscope_mp" );
